...
fn pbr_metalrough(light: light_data, mat: pbr_material, vert: vertex_output) -> vec4f {
	// Outgoing light direction (vector from world-space fragment position to the "eye").
	let Lo = normalize(utility.camera.position - vert.position);
	...
	// Fresnel reflectance at normal incidence (for metals use albedo).
	let F0 = mix(Fdielectric, mat.albedo, mat.metalness).rgb;

	// Direct lighting calculation for analytical lights.
	{
		let i = 0;
		let Li = -light.direction; // TODO: We can do better direction calculation
		let Lradiance = light.color.rgb * light.intensity * light.color.a; // TODO: We can do better attenuation calculation
		...
		let diffuseBRDF = kd * mat.albedo.rgb;
...
fn pbr_simple_ambient(light: light_data, mat: pbr_material, vert: vertex_output) -> vec4f {
	return vec4(vec3(0.03) * mat.albedo.rgb * mat.ambient_occlusion, mat.albedo.a);
}
