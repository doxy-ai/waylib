...
void upload_buffer(waylib_state state, buffer& buffer, WGPUBufferUsageFlags usage /*= wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::Storage*/, bool free_cpu_data /*= false*/) {
...
void upload_buffer(waylib_state state, buffer* buffer, WGPUBufferUsageFlags usage /*= WGPUBufferUsage_CopyDst | WGPUBufferUsage_Storage*/, bool free_cpu_data /*= false*/){
	upload_buffer(state, *buffer, usage, free_cpu_data);
}

buffer create_buffer(waylib_state state, std::span<std::byte> data, WGPUBufferUsageFlags usage /*= wgpu::BufferUsage::CopyDst | wgpu::BufferUsage::Storage*/, WAYLIB_OPTIONAL(std::string_view) label /*= {}*/) {
	buffer out = {
		.size = data.size(),
		.offset = 0,
...
buffer create_buffer(waylib_state state, void* data, size_t size, WGPUBufferUsageFlags usage /*= WGPUBufferUsage_CopyDst | WGPUBufferUsage_Storage*/, const char* label /*= nullptr*/) {
...
void* buffer_map(waylib_state state, buffer& buffer, WGPUMapModeFlags mode /*= wgpu::MapMode::None*/) {
...
void* buffer_map(waylib_state state, buffer* buffer, WGPUMapModeFlags mode /*= WGPUMapMode_None*/) {
	return buffer_map(state, *buffer, mode);
}

const void* buffer_map_const(waylib_state state, buffer& buffer, WGPUMapModeFlags mode /*= wgpu::MapMode::None*/) {
	if(mode != wgpu::MapMode::None) {
		bool done = false;
		// TODO: Update API
		auto handle = buffer.data.mapAsync(mode, buffer.offset, buffer.size, [&](wgpu::BufferMapAsyncStatus status) {
...
void buffer_copy_record_existing(WGPUCommandEncoder _encoder, buffer& dest, const buffer& source) {
	wgpu::CommandEncoder encoder = _encoder;
	encoder.copyBufferToBuffer(source.data, source.offset, dest.data, dest.offset, dest.size);
}
void buffer_copy_record_existing(WGPUCommandEncoder encoder, buffer* dest, const buffer* source) {
	buffer_copy_record_existing(encoder, *dest, *source);
}

void buffer_copy(waylib_state state, buffer& dest, const buffer& source) {
...
void buffer_download(waylib_state state, buffer& buffer, bool create_intermediate_buffer /*= true*/) {
...
void upload_computer(waylib_state state, computer& compute, WAYLIB_OPTIONAL(std::string_view) label /*= {}*/) {
...
void upload_computer(waylib_state state, computer* compute, const char* label /*= nullptr*/) {
...
void quick_dispatch(waylib_state state, std::span<buffer> buffers, std::span<texture> textures, shader compute_shader, vec3u workgroups) {
	computer compute {
...
void quick_dispatch(waylib_state state, buffer* buffers, size_t buffer_size, texture* textures, size_t texture_size, shader compute_shader, vec3u workgroups) {
